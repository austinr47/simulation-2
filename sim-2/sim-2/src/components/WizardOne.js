import React,  { Component } from 'react';
import {Link} from 'react-router-dom'
import { connect } from 'react-redux';
import {updatePropertyName, updatePropertyDescription} from '../ducks/reducer';

  class WizardOne extends Component {

    render(){
        const { updatePropertyName, updatePropertyDescription } = this.props;
        return(
            <div className="parent-div bck-color">
                <div className="header-bck">
                    <div>
                        <img alt="house" src='data:image/png;base64,'/>
                        Houser DashBoard
                    </div>
                </div>
                <div className="vert-align">
                    <p>Property Name</p> <br />
                
                    <input placeholder={`${this.props.propertyName}`} onChange={(event) => updatePropertyName(event.target.value)} />

                     <br/>

                    <p>Property Description</p> <br />

                    <input placeholder={`${this.props.propertyDescription}`} onChange={(event) => updatePropertyDescription(event.target.value)} />
                    
                    <Link to="/wTwo"><button className="margin-btn"> Next Step </button></Link>
                    {/* {console.log(this.props.propertyDescription)} */}
                </div>
            </div>
        )
    }
}

// simulation 2 43H
// simulation 2 43G
const mapStateToProps = state => {
    const { propertyName, propertyDescription} = state;
    return{
        propertyName,
        propertyDescription
    }

}
// simulation 2 43C
export default connect(mapStateToProps, {updatePropertyName, updatePropertyDescription})(WizardOne); 